check_dimer<-function(primer,outputfile){
	...
	cmd<-lapply(1:(nrow(x)-1),function(i){
		sapply((i+1):nrow(x),function(j){
			cmd<-paste("D:/Install/primer3-win-bin-2.3.6/ntthal.exe -s1",x[i,2],"-s2",x[j,2],"-dv 3.2 -n 0.85 -mv 52 -d 40 -a END1 -path D:/Install/primer3-win-bin-2.3.6/primer3_config/")
		})
	})
	cmd<-unlist(cmd)
	...
	setwd("E:/primer_dimer/")
	n<-20
	idx<-rep(1:20,each=as.integer(length(cmd)/n)+1)
	idx<-idx[1:length(cmd)]
	cmd1<-split(cmd,idx)
	name1<-split(name,idx)

	xx<-mapply(function(x,y,i){
		y<-paste("echo ##",y,">>",paste("E:/primer_dimer/part_",i,".txt",sep=""))
		x<-paste(x,">>",paste("E:/primer_dimer/part_",i,".txt",sep=""))
		cmd<-as.character(t(cbind(y,x)))
		writeLines(cmd,paste("heterodimer_cmd_part_",i,".bat",sep=""))	
	},cmd1,name1,1:length(cmd1),SIMPLIFY=F)

	xx<-sapply(1:n,function(x){
		paste("call heterodimer_cmd_part_",x,".bat",sep="")
	})
	writeLines(xx,"batch_run.bat")
	system("batch_run.bat",intern=T,wait=F,ignore.stdout = T)
	setwd("E:/primer_dimer/")
	file<-list.files(pattern="*part_\\d+.txt") #file<-system("ls part*.txt",intern=T)
...
prepare_bat<-function(tmp_dir,primer,primer3dir){
	cwd<-getwd();
	id<-c(paste(primer[,1],"_F",sep=""),paste(primer[,1],"_R",sep=""))
	seq<-c(primer[,2],primer[,3])
	x<-cbind(id,seq)
	...
	cmd<-lapply(1:(nrow(x)-1),function(i){
		sapply((i+1):nrow(x),function(j){
			cmd<-paste(primer3dir,"/ntthal.exe -s1 ",x[i,2]," -s2 ",x[j,2]," -dv 3.2 -n 0.85 -mv 52 -d 40 -a END1 -path ",primer3dir,"/primer3_config/",sep="") # TODO primer3°²×°Â·¾¶
		})
	})
	cmd<-unlist(cmd)
	name<-lapply(1:(nrow(x)-1),function(i){
	...
	xx<-mapply(function(x,y,i){
		y<-paste("echo ##",y,">>",paste(tmp_dir,"/part_",i,".txt",sep=""))
